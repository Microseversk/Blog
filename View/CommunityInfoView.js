...
class CommunityInfoView {
  getFiltersValues(page = 1) {
    let tags = Array.from(document.querySelector('#filter-tags').selectedOptions).map(
      (option) => option.value
    );
    return {
  ...
  renderFiltersValues(params) {
    params.tags.forEach((tagId) => {
      const option = document
        ...
        .querySelector(`option[value="${tagId}"]`);
      if (option) {
        option.selected = true;
      }
    });
    document.querySelector('#filter-sorting-by').value = params.sorting;
    document.querySelector('#filter-page-post-size').value =
      params.size !== null ? params.size : CURRENT_PAGE_SIZE;
  }
  ...
  renderInfo(adminTemplate, info, communityId, callback) {
    ...
    admins.forEach((admin) => {
      ...
      adminContainer.querySelector('#admin-template-image').src =
        admin.gender === MALE ? MAN : WOMAN;
    ...
    document.querySelector('#communityInfo-subs-counter').innerText =
      info.subscribersCount + ' подписчиков';
    document.querySelector('#communityInfo-private-status').innerText =
      info.isClosed === true ? 'Тип сообщества: закрытое' : 'Тип сообщества: открытое';
  ...
  renderPosts(template, posts) {
    ...
    posts.posts.forEach((post) => {
      ...
      cardTemplate.querySelector('#post-template-date').textContent =
      ...
      if (post.commentsCount > 0) {
        cardTemplate
          .querySelector('#post-template-comments-icon')
          .classList.add('bi-chat-left-fill');
      } else {
      ...
      cardTemplate.querySelector('#post-template-reading-time').textContent =
        'Время чтения: ' + post.readingTime + ' мин.';
      cardTemplate.querySelector('#post-template-community').textContent =
      ...
      cardTemplate.querySelector('#post-template-like-icon').addEventListener('click', async () => {
        ...
        if (method === 'POST') {
          ...
          cardTemplate.querySelector('#post-template-like-icon').style.color = LIKE_COLOR;
          cardTemplate.querySelector('#post-template-likes-count').textContent =
            parseInt(cardTemplate.querySelector('#post-template-likes-count').textContent) + 1;
          try {
            const response = fetch(
              `https://blog.kreosoft.space/api/post/${cardTemplate.querySelector('#post-template-title').getAttribute('data-id')}/like`,
              {
                method: method,
                headers: {
