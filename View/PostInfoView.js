...
class PostInfoView {
  ...
  renderPostInfo(post) {
    ...
    document.querySelector('#post-template-date').textContent =
      ' - ' + normalizeDateTime(post.createTime);
    ...
    document.querySelector('#post-template-reading-time').textContent =
      'Время чтения: ' + post.readingTime + ' мин.';
    document.querySelector('#post-template-community').textContent =
      post.communityName != null ? 'в сообществе ' + '"' + post.communityName + '"' : '';
    document.querySelector('#post-template-like-icon').style.cursor =
      getToken() !== null ? 'pointer' : 'default';
    document.querySelector('#post-template-like-icon').addEventListener('click', async () => {
      if (getToken() === null) {
        return;
      }
      let method = document
        .querySelector('#post-template-like-icon')
      ...
      if (method === 'POST') {
        ...
        document.querySelector('#post-template-like-icon').style.color = LIKE_COLOR;
        document.querySelector('#post-template-likes-count').textContent =
          parseInt(document.querySelector('#post-template-likes-count').textContent) + 1;
        try {
          const response = fetch(
            `https://blog.kreosoft.space/api/post/${document.querySelector('#post-template-title').getAttribute('data-id')}/like`,
            {
              method: method,
              headers: {
  ...
  renderComments(
    ...
    rootComments.forEach((comment) => {
      ...
      commentContainer
        .querySelector('#post-info-comment-btn-open-sub-comments')
        .addEventListener('click', () => {
          commentContainer
            .querySelector('#post-info-comment-btn-open-sub-comments')
            .classList.add('d-none');
          commentContainer
            .querySelector('#post-info-subComments-holder')
            .classList.remove('d-none');
          commentContainer
            .querySelector('#post-info-comment-btn-hide-sub-comments')
            .classList.remove('d-none');
        });
      ...
      if (comment.subComments.length === 0) {
        ...
        comment.subComments.forEach((subComment) => {
          let subCommentContainer = this.initComment(
            subComment,
            subCommentTemplate,
            checkResponse,
            postId
          ...
          commentContainer
            .querySelector('#post-info-subComments-holder')
            .appendChild(subCommentContainer);
        });
      }
  ...
  initComment(comment, commentTemplate, checkResponse, postId) {
    ...
    commentContainer.querySelector('#post-info-comment-date').innerHTML = normalizeDateTime(
      comment.createTime
    );

    if (comment.modifiedDate !== null && comment.deleteDate === null) {
      commentContainer
        .querySelector('#post-info-comment-rewrite-status')
        .classList.remove('d-none');
      commentContainer.querySelector('#post-info-comment-rewrite-status').title = normalizeDateTime(
    ...
    if (comment.deleteDate !== null) {
      ...
      commentContainer.querySelector('#post-info-comment-author-name').innerText =
        '[Комментарий удалён]';
      commentContainer.querySelector('#post-info-comment-text').title = normalizeDateTime(
        comment.deleteDate
      );
    }
    ...
    commentContainer
      .querySelector('#post-info-comment-trash-icon')
    ...
    commentContainer
      .querySelector('#post-info-comment-pencil-icon')
      .addEventListener('click', async () => {
        let refactorInputs = document
          .querySelector('#post-info-comments-container')
          .querySelectorAll('#post-info-comment-refactor');
        let answerInputs = document
          .querySelector('#post-info-comments-container')
          .querySelectorAll('#post-info-comment-answer');
        ...
        if (
          commentContainer.querySelector('#post-info-comment-refactor').classList.contains('d-none')
        ) {
          commentContainer.querySelector('#post-info-comment-refactor').classList.remove('d-none');
          commentContainer.querySelector('#post-info-comment-refactor-input').value =
            comment.content;

          commentContainer
            .querySelector('#post-info-comment-btn-refactor-send')
            .addEventListener('click', async () => {
          ...
          commentContainer
            .querySelector('#post-info-comment-refactor')
            .querySelector('input').value = '';
        }
      });

    commentContainer
      .querySelector('#post-info-comment-btn-answer')
      .addEventListener('click', () => {
        let refactorInputs = document
          .querySelector('#post-info-comments-container')
          .querySelectorAll('#post-info-comment-refactor');
        ...
        if (
          commentContainer.querySelector('#post-info-comment-answer').classList.contains('d-none')
        ) {
          commentContainer.querySelector('#post-info-comment-answer').classList.remove('d-none');

          commentContainer
            .querySelector('#post-info-comment-btn-answer-send')
            .addEventListener('click', async () => {
