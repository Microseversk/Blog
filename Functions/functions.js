...
export async function addAddressSelect(parentAddress, currentAddressLevel, addressInfo) {
  ...
  let placeholderOption = new Option(
    'Не выбрано',
    {
      objectId: '',
      text: 'Не выбрано',
  ...
  $(newSelect).on('select2:select', async function (e) {
    ...
    if (newLabelId === 'create-post-label-1') {
      ...
      document.querySelector('#' + newLabelId).innerText =
        address.levelText !== '' ? address.levelText : 'Следующий элемент адреса';
    }
    ...
    const nextData = await (
      await fetch('https://blog.kreosoft.space/api/address/search?parentObjectId=' + address.id)
    ).json();
