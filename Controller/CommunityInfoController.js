...
class CommunityInfoController {
  ...
  async init() {
    ...
    const posts = await this.model.getCommunityPosts(
      this.communityId,
      this.currentQuery.toString()
    );
  ...
  async switchPage(param) {
    if (param === GO_NEXT_PAGE) {
      this.model.currentPage =
        this.model.currentPage < this.model.currentPageCount
          ? this.model.currentPage + 1
          : this.model.currentPage;
    } else if (param === GO_PREVIOUS_PAGE) {
      this.model.currentPage =
        this.model.currentPage > 1 ? this.model.currentPage - 1 : this.model.currentPage;
    }
